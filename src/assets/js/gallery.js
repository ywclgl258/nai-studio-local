...
function render() {
    ...
    if (items.length === 0) {
        ...
        empty.textContent = _historyFilter === 'favorites' ? '还没有收藏' : '还没有生成图片';
...
function renderItem(item) {
    ...
    // v0.8 浮层 meta（仅 v2 渲染时显示）
    if (document.querySelector('.app-shell[data-shell="v2"]')) {
        const meta = document.createElement('div');
        meta.className = 'v2-meta';
        const seed = (item.seed ?? item.params?.seed ?? '').toString();
        const model = (item.model ?? '').replace(/^nai-diffusion-/, '').replace(/-curated$/, '');
        meta.innerHTML = `<span class="seed">#${seed || '—'}</span><span class="model">${model || ''}</span>`;
...
// v0.8 历史 tabs 过滤（v2 only）
let _historyFilter = 'all';
function getFilteredItems() {
    if (_historyFilter === 'favorites') return _allItems.filter(x => x.is_favorite);
    if (_historyFilter === 'recent') return _allItems.slice(0, 12);
    return _allItems;
}
function updateHistoryTabCounts() {
    const all = _allItems.length;
    const fav = _allItems.filter(x => x.is_favorite).length;
    const recent = Math.min(12, all);
    const a = document.getElementById('historyAllCount');
    const f = document.getElementById('historyFavCount');
    const r = document.getElementById('historyRecentCount');
    if (a) a.textContent = all;
    if (f) f.textContent = fav;
    if (r) r.textContent = recent;
}
function setupHistoryTabs() {
    if (!document.querySelector('.app-shell[data-shell="v2"]')) return;
    document.querySelectorAll('#historyTabs button').forEach(b => {
        b.addEventListener('click', () => {
            _historyFilter = b.dataset.historyTab;
            document.querySelectorAll('#historyTabs button').forEach(x =>
...
// v0.8 FAB 溢出菜单
function setupFabOverflow() {
    const btn = document.getElementById('mainFabOverflowBtn');
    const menu = document.getElementById('mainFabOverflowMenu');
    if (!btn || !menu) return;
...
export function initGallery() {
    setupHistoryTabs();   // v0.8 history tabs
    setupFabOverflow();   // v0.8 FAB 溢出菜单
    // Sidebar toggle
    ...
    // v0.8 复制提示词 / 发送到 Director（已从顶栏一级按钮收纳进 FAB 溢出菜单，故直接在此绑定，不再转发到已不存在的 main*Btn）
    document.getElementById('fabCopyPromptBtn')?.addEventListener('click', () => {
        const it = getActive(); if (it) copyToClipboard(it.prompt || '');
        document.getElementById('mainFabOverflowMenu')?.classList.add('hidden');
    });
    document.getElementById('fabDirectorBtn')?.addEventListener('click', () => {
        const it = getActive(); if (it) applyToDirector(it);
        document.getElementById('mainFabOverflowMenu')?.classList.add('hidden');
    });
