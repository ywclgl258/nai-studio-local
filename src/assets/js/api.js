...
export const api = {
    ...
    // Real-ESRGAN 无损放大
    upscaleStatus:        () => request('GET',  '/api/upscale.php?action=status'),
    upscaleDownloadStatus:() => request('GET',  '/api/upscale.php?action=download_status'),
    upscaleInstall:       () => request('POST', '/api/upscale.php?action=download_start'),
    upscaleImage:         (generationId, scale, saveToGallery) => request('POST', '/api/upscale.php?action=upscale', {
        body: { generation_id: generationId, scale, save_to_gallery: saveToGallery ? 1 : 0 }
    }),
