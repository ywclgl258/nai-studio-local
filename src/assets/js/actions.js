...
let _realesrganPoll = null;
function initRealesrgan() {
    const btnInstall = document.getElementById('actionInstallRealesrgan');
    const btnRecheck = document.getElementById('actionRecheckRealesrgan');
    if (!btnInstall) return;

    refreshRealesrgan();

    btnInstall.addEventListener('click', async () => {
        if (!confirm('下载 Real-ESRGAN v0.2.5.0 (~50MB) 并安装到本地？\n\n下载期间可以关浏览器（后台继续）。\n失败原因多半是 GitHub 限流 —— 换个时段或开代理。')) return;
        btnInstall.disabled = true;
        btnInstall.textContent = '下载中…';
        try {
            const r = await api.upscaleInstall();
            if (r.already_ready) {
                toast('✓ Real-ESRGAN 已就绪', { type: 'success' });
                btnInstall.disabled = false;
                btnInstall.textContent = '✓ 已安装';
                setTimeout(() => btnInstall.textContent = '⬇ 重新下载', 2000);
            } else {
                toast('已开始下载（后台跑，可关浏览器）', { type: 'info', duration: 5000 });
                pollRealesrgan();
            }
        } catch (e) {
            toast('启动下载失败: ' + e.message, { type: 'error' });
            btnInstall.disabled = false;
            btnInstall.textContent = '⬇ 一键下载安装';
        }
    });

    btnRecheck.addEventListener('click', refreshRealesrgan);
}

async function refreshRealesrgan() {
    const statusEl = document.getElementById('realesrganStatus');
    const btnInstall = document.getElementById('actionInstallRealesrgan');
    if (!statusEl) return;
    try {
        const r = await api.upscaleStatus();
        renderRealesrganStatus(r.status);
    } catch (e) {
        statusEl.innerHTML = `<span style="color:var(--danger)">查询失败: ${e.message}</span>`;
    }
}

function renderRealesrganStatus(s) {
    const statusEl = document.getElementById('realesrganStatus');
    const btnInstall = document.getElementById('actionInstallRealesrgan');
    const btnRecheck = document.getElementById('actionRecheckRealesrgan');
    if (!s) return;
    if (s.ready) {
        statusEl.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px">
                ...
                <div>
                    <div style="font-weight:600;color:#22c55e">已就绪</div>
                    <div style="font-size:11px;color:var(--text-muted);margin-top:2px">
                        模型: <code>${s.model_name}</code> · 已用: ${s.total_size_mb}MB
                    </div>
                </div>
            </div>
        `;
        btnInstall.textContent = '🔄 重新下载';
        btnInstall.disabled = false;
    } else {
        const missing = [];
        if (!s.binary) missing.push('可执行文件');
        if (!s.model_bin) missing.push('模型 .bin');
        if (!s.model_param) missing.push('模型 .param');
        statusEl.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px">
                <span style="color:var(--warning);font-size:18px">○</span>
                <div>
                    <div style="font-weight:600;color:var(--warning)">未安装</div>
                    <div style="font-size:11px;color:var(--text-muted);margin-top:2px">
                        缺失: ${missing.join('、') || '全部'}
                    </div>
                </div>
        ...
        `;
        btnInstall.textContent = '⬇ 一键下载安装 (~50MB)';
        btnInstall.disabled = false;
    }
    btnRecheck?.classList.remove('loading');
}

function pollRealesrgan() {
    if (_realesrganPoll) clearInterval(_realesrganPoll);
    const fill = document.getElementById('realesrganBarFill');
    const txt  = document.getElementById('realesrganProgress');
    const btnInstall = document.getElementById('actionInstallRealesrgan');
    btnInstall.disabled = true;
    btnInstall.textContent = '下载中…';
    _realesrganPoll = setInterval(async () => {
        try {
            const r = await api.upscaleDownloadStatus();
            ...
            if (s === 'idle' || s === 'starting') {
                ...
                fill.style.width = pct + '%';
                txt.textContent = '📥 ' + (r.message || '下载中...');
            } else if (s === 'extracting') {
                fill.style.width = '90%';
                txt.textContent = '📦 解压整理中...';
            } else if (s === 'done') {
                fill.style.width = '100%';
                txt.innerHTML = `<span style="color:#22c55e">✓ 安装完成！模型: ${r.models || 0} 个文件</span>`;
                clearInterval(_realesrganPoll);
                _realesrganPoll = null;
                refreshRealesrgan();
                toast('🎉 Real-ESRGAN 安装完成！可以在画廊里点 🔍 放大图片了', { type: 'success', duration: 6000 });
                btnInstall.disabled = false;
                btnInstall.textContent = '✓ 已安装';
                setTimeout(() => btnInstall.textContent = '⬇ 重新下载', 3000);
            } else if (s === 'error') {
                fill.style.width = '100%';
                fill.style.background = 'var(--danger)';
                txt.innerHTML = `<span style="color:var(--danger)">✗ 失败: ${r.message || '未知错误'}</span>`;
                clearInterval(_realesrganPoll);
                _realesrganPoll = null;
                btnInstall.disabled = false;
            ...
            txt.innerHTML = `<span style="color:var(--danger)">查询失败: ${e.message}</span>`;
        }
    }, 1500);
}