...
async function reclassifyOne(p) {
    try {
        ...
        if (r && r.cn) {
            ...
            // 没缓存就调 en→zh
            const trR = await fetch(`/nai-studio/api/danbooru.php?action=translate&q=${encodeURIComponent(p.name)}`).then(r => r.json());
...
async function fillTranslate() {
    ...
    for (const p of need) {
        ...
        if (overrides[key]) {
            ...
            try {
                const r = await fetch(`/nai-studio/api/danbooru.php?action=translate&q=${encodeURIComponent(p.name)}`).then(r => r.json());
                if (r && r.cn) p.cn = r.cn;
            } catch {}
        }
        done++;
