...
### 2. 启动
双击根目录的 `start.bat`，看到 `Service ready` 即可
...
### 5. 关闭
双击根目录的 `stop.bat`（或直接关浏览器，server 还在后台跑）
...
- `start.bat` — 5 步启动 + CRLF + goto+label（从 tools/ 搬到根目录）
- `stop.bat` — taskkill 杀 8080（从 tools/ 搬到根目录）
