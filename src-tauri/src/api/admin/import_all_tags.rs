//! /api/admin/import-all-tags -- 批量导入 Danbooru tag
//!
//! Phase 3.4: 基础版 — 在线分页拉 + 写 cache
//! Phase 4: 增量 + 去重 + 翻译调度
...
async fn run_worker(_cancel: std::sync::Arc<std::sync::atomic::AtomicBool>) {
    update(NAME, |j| {
        j.message = "模拟导入（Phase 3.4 占位）...".to_string();
    });
    let total: i64 = 50;
    for i in 0..total {
        ...
        tick(NAME, i + 1, total);
        if (i + 1) % 10 == 0 {
            update(NAME, |j| { j.message = format!("导入中 {}/{}", i + 1, total); });
        }
    }
    update(NAME, |j| {
