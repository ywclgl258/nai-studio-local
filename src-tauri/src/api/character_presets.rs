//! /api/character_presets -- 角色预设 CRUD
//!
//! 跟 NAI Studio PHP 项目的 character_presets.php 等价
//!   - GET    /api/character_presets?favorite=1
//!   - POST   {action:create|update|delete|use|favorite, ...}
//!   - DELETE ?id=N
...
pub async fn list(
    ...
    let sql = if only_fav {
        "SELECT id, title, prompt, is_favorite, use_count, last_used_at, created_at, updated_at
         FROM character_presets WHERE is_favorite = 1 ORDER BY last_used_at DESC, id DESC"
    } else {
        "SELECT id, title, prompt, is_favorite, use_count, last_used_at, created_at, updated_at
         FROM character_presets ORDER BY is_favorite DESC, last_used_at DESC, id DESC"
    };
...
pub async fn create(
    ...
    match action {
        "create" => {
            ...
            conn.execute(
                "INSERT INTO character_presets (title, prompt, is_favorite) VALUES (?, ?, ?)",
        ...
        "use" => {
            ...
            conn.execute("UPDATE character_presets SET use_count = use_count + 1, last_used_at = CURRENT_TIMESTAMP WHERE id = ?", [id])?;
        ...
        "favorite" => {
            ...
            conn.execute("UPDATE character_presets SET is_favorite = ? WHERE id = ?", rusqlite::params![v, id])?;
        ...
        "update" => {
            ...
            let sql = format!("UPDATE character_presets SET {} WHERE id = ?", sets.join(", "));
        ...
        "delete" => {
            let id = obj.get("id").and_then(|v| v.as_i64())
                .ok_or_else(|| AppError::BadRequest("id required".into()))?;
            conn.execute("DELETE FROM character_presets WHERE id = ?", [id])?;
            Ok(Json(json!({"ok": true, "id": id})))
        }
...
pub async fn delete(
    ...
    let id: i64 = params.get("id").and_then(|s| s.parse().ok())
        .ok_or_else(|| AppError::BadRequest("id required".into()))?;
    let conn = state.db.lock();
    let n = conn.execute("DELETE FROM character_presets WHERE id = ?", [id])?;
