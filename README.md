...
[![No Apache](https://img.shields.io/badge/no-XAMPP-green.svg)](#-快速开始)
...
只需要 **PHP 8.0+**（推荐 8.2）和浏览器。**不需要 XAMPP / MySQL / Apache**——NAI Studio 用 PHP 内置服务器 + SQLite 单文件。
...
如果 `php` 不在 PATH，可以装到 `C:\xampp\php\php.exe`、`C:\php\php.exe` 或 `C:\Program Files\php\php.exe`，start.bat 会自动检测。
...
1. 检查 PHP + SQLite DB
2. 杀掉占用 8080 端口的旧进程
3. 后台启动 PHP 内置服务器（脱离窗口生命周期）
4. 等端口就绪 → 写 PID 文件
5. 打开浏览器到 http://127.0.0.1:8080/nai-studio/
...
│   │   ├── backend.php        # 后端服务控制（status/stop）
...
│   └── favicon.*
├── data/
│   ├── nai-studio.db          # SQLite 数据库（含 API key + 全部运行时数据）
│   ├── schema_sqlite.sql      # 表结构
│   └── backups/               # 自动备份
├── tools/
│   ├── start.bat              # 一键启动
│   ├── stop.bat               # 一键停止
│   ├── php_server.cmd         # PHP 实际启动命令（被 start.bat 通过 vbs 调用）
│   ├── fetch_all_tag_images.php  # 批量预下载标签预览图
│   └── migrate_*.php          # 数据库迁移脚本
├── translate-server/          # 独立本地翻译服务（Node.js + OPUS-MT）
│   ├── start.bat              # 双击启动
│   ├── stop.bat
│   ├── server.js
│   └── package.json
...
NAI Studio 默认用 **SQLite 单文件** `data/nai-studio.db`。
无需任何外部服务，**copy 文件就能备份**。

如果你要换成 MariaDB / MySQL：改 `src/config.php` 里的 `driver` + `dsn` + `user` + `password`，然后 `php tools/migrate_mysql_to_sqlite.php`（或反向）。
...
**Q: 启动报 "PHP not found"？**
A: 把 PHP 加到 PATH，或装到 `C:\xampp\php\php.exe` 这种 start.bat 自动检测的路径。
